...
namespace StudentAttendence.Controller
{
    class Section
    {
        ...
        public void GetAllSectionList(int departmentID, DataGridView dataGridView)
        {
            ...
            {
                connect.Open();
                MySqlCommand mySqlCommand = new MySqlCommand("SELECT sec.`sec_id`, d.`dept_name`, sec.`sec_name`, sem.`sem_name` FROM `db_attendance_assistance`.`tbl_section` AS sec INNER JOIN `tbl_department` AS d ON sec.`dept_id` = d.`dept_id` INNER JOIN `tbl_semester` AS sem ON sec.`sem_id` = sem.`sem_id` WHERE d.`dept_id` = @departmentID; ", this.connect);
                mySqlCommand.Parameters.Clear();
        ...
        public void GetSingleSection(ComboBox semester, TextBox sectionName)
        {
            ...
            {
                connect.Open();
                MySqlCommand mySqlCommand = new MySqlCommand("SELECT `sec_id`, `sec_name`, `teacher_id`, `sem_id` FROM `db_attendance_assistance`.`tbl_section` WHERE `sec_id` = " + sectionID + "; ", this.connect);
                MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(mySqlCommand);
                DataTable dataTable = new DataTable();
                mySqlDataAdapter.Fill(dataTable);
        ...
        public bool UpdateSection(int department_id, int semester_id, String name)
        {
            ...
            {
                connect.Open();
                MySqlCommand mySqlCommand = new MySqlCommand("UPDATE `db_attendance_assistance`.`tbl_section` SET `dept_id` = @department_id, `sec_name` = @name, `sem_id` = @semester_id WHERE `sec_id` = " + this.sectionID + "; ", connect);
                mySqlCommand.Parameters.Clear();
                mySqlCommand.Parameters.Add(new MySqlParameter("@department_id", department_id));
